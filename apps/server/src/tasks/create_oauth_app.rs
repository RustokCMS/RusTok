...
impl Task for CreateOAuthAppTask {
    ...
    async fn run(&self, app_context: &AppContext, vars: &Vars) -> loco_rs::Result<()> {
        let name = vars.cli_arg("name").unwrap_or_else(|_| "Development App".to_string());
        let slug = vars.cli_arg("slug").unwrap_or_else(|_| "dev-app".to_string());
        ...
        let tenant = Tenants::find()
            .one(db)
            .await?
            .ok_or_else(|| loco_rs::Error::Message("No tenant found. Run seeds first.".to_string()))?;
        ...
        let input = CreateOAuthAppInput {
            ...
            redirect_uris: vec!["http://localhost:3000/api/auth/callback".to_string(), "http://localhost:1420".to_string()],
            scopes: vec!["openid".to_string(), "profile".to_string(), "email".to_string(), "offline_access".to_string()],
            grant_types: vec!["authorization_code".to_string(), "refresh_token".to_string()],
        };

        let result = OAuthAppService::create_app(db, tenant.id, input).await.map_err(|e| {
            loco_rs::Error::Message(format!("Failed to create app: {}", e))
        })?;
