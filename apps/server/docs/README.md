...
- Auth/RBAC consistency: REST RBAC extractors (`extractors/rbac.rs`) используют общую wildcard-семантику `resource:manage` через `rustok_rbac::services::permission_policy::has_effective_permission_in_set`, без локального дублирования permission-логики.
