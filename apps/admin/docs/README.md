...
- `/modules` теперь точно так же опирается на server-driven moderation contract для richer lifecycle decisions: detail panel показывает и запускает `request_changes`, `hold` и `resume`, рендерит persisted metadata для `changes_requested` / `on_hold` (`reason`, `reason_code`, actor, timestamp, `heldFromStatus`) и подсказывает правильный next-step path без локальных policy-эвристик.
