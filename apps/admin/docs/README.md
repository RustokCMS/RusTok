...
- `Publish lifecycle` теперь также показывает отдельный `Follow-up gates` блок по `compile_smoke`, `targeted_tests` и `security_policy_review`, чтобы оператор видел внешние async/manual gates отдельно от базовой artifact/manifest validation; для всех трёх stages панель теперь подсказывает `cargo xtask module stage-run ...`, но для `security_policy_review` этот path требует explicit `--confirm-manual-review` и остаётся operator-assisted review, а не полностью автоматическим security scanner. Live terminal stage updates (`passed` / `failed` / `blocked`) теперь также требуют structured `reason_code`, и panel body hints уже показывают это как часть operator contract.
