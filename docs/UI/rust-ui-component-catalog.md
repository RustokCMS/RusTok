...
Оба приложения (`apps/admin` и `apps/next-admin`) используют один и тот же набор shadcn CSS-переменных (`--background`, `--primary`, `--destructive`, `--border` и т.д.) и одинаковый `tailwind.config.js`.
...
Оба приложения определяют одинаковый набор CSS-переменных в своём CSS entry point:
...
Переменные: `--background`, `--foreground`, `--card`, `--card-foreground`, `--primary`, `--primary-foreground`, `--secondary`, `--muted`, `--accent`, `--destructive`, `--border`, `--input`, `--ring`, `--radius`, `--sidebar-*`.
...
1. `Alert` / `AlertDialog` — нужны для подтверждений (порт shadcn alert.tsx / alert-dialog.tsx)
2. `Combobox` / `MultiSelect` — для фильтрации данных
3. `Skeleton` в Leptos — для loading states
4. `Dialog` в Leptos — нативная реализация или Leptonic как временное решение
