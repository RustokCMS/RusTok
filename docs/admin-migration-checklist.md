...
### Зафиксированные зависимости для admin (добавлено)

Устанавливаем в `apps/admin` и фиксируем как рабочий baseline:

- `gloo-storage` — единый и безопасный слой для работы с LocalStorage/SessionStorage.
- `serde_urlencoded` — корректная сериализация query/form параметров без ручной склейки строк.
- `chrono-humanize` — человекочитаемое представление времени в UI (`5 minutes ago`, `2 hours ago`).
- `leptos-chartistry` — графики и метрики для dashboard/observability экранов.

### Кандидаты "на будущее" (зафиксировать, чтобы не потерять)

- `totp-rs` — 2FA (TOTP), QR onboarding, секреты и otpauth-поток.
- `regex` — расширенная клиентская валидация (где недостаточно типовых правил форм).
- `humansize` — удобный формат размеров файлов/артефактов/логов.
- `form_urlencoded` — низкоуровневая сборка query-string в edge-кейсах.

> Принцип: подключаем кандидат в production-код только при появлении конкретной фичи и тестового сценария.
