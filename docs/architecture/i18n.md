...
- `flex` is part of the same multilingual cleanup: standalone schema copy now belongs in parallel localized records instead of inline base-row fields, attached-mode `user`, `product`, `order`, and `topic` custom values already use parallel localized rows, and the remaining follow-up scope is now concentrated in standalone entry-value storage plus cleanup/backfill of transitional inline locale-aware payloads rather than in donor-path parity.
