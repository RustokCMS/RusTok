...
- Attached-mode `flex` field definitions now also carry explicit `is_localized` semantics, the shared attached-value contract now lives in `crates/flex`, and live attached-value paths (`user`, `product`, `order`, `topic`) persist locale-aware values in `flex_attached_localized_values` while keeping legacy inline metadata only as transitional fallback during migration and cleanup.
...
- `flex` is part of the same multilingual cleanup: standalone schema copy now belongs in parallel localized records instead of inline base-row fields, standalone entry values are now split into `flex_entries` (shared payload) plus `flex_entry_localized_values` (locale-aware payload), and attached-mode `user`, `product`, `order`, and `topic` custom values already use parallel localized rows. The remaining follow-up scope is now limited to cleanup/backfill of transitional inline locale-aware JSON payloads and future public standalone surfaces, not donor-path parity.
