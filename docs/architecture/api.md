...
| OpenAPI | `/api/openapi.json`, `/api/openapi.yaml` | Машиночитаемая спецификация REST API (генерируется через `utoipa`) |
| Health | `/health`, `/health/live`, `/health/ready`, `/health/modules` | Статус процесса, readiness и health модулей |
