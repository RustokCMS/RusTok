...
### 2.15 🟡 ВАЖНО: «Компиляционные guardrails» против типичных ошибок AI

**Контекст.** Ниже фиксируем рекомендации из архитектурного обсуждения в формате «берём/не берём» с опорой на текущее состояние RusToK, чтобы не плодить дубли документации и не повторять уже реализованное.

| Паттерн | Что уже есть в репозитории | Решение по RusToK | Следующий шаг |
|---|---|---|---|
| **ServiceOutcome<T>** (сервис обязан вернуть и `data`, и `events`) | Единого compile-time контракта для service output пока нет; публикация событий контролируется паттернами (`publish_in_tx`) и код-ревью. | **Берём частично**: как опциональный контракт для mutating use-cases в domain-модулях, без массового breaking-рефакторинга сразу. | Ввести `ServiceOutcome<T, E = DomainEvent>` в `rustok-core` и начать с pilot в `commerce`/`content` сервисах с write-операциями. |
| **TenantScope / Scoped repository** (невозможно запросить без tenant фильтра) | Изоляция тенантов обеспечивается на уровне сервисов/репозиториев и интеграционных тестов, но нет общего compile-time wrapper для всех SeaORM-запросов. | **Берём**: как целевой стандарт для read/write операций, где tenant-контекст обязателен. | Добавить в `rustok-core` typed scope-обёртку и migration guide для модулей; внедрять поэтапно без big-bang. |
| **Модульные границы через Cargo dependencies** | Уже используется: модуль не может импортировать crate, не объявленный в `Cargo.toml`; дополнительно есть module manifest validation. | **Уже реализовано (оставляем как есть)**. | Добавить lightweight CI-check на запрещённые cross-domain зависимости для критичных пар модулей (например, blog → commerce). |
| **`docs/ai-examples` как эталонные живые примеры** | Есть `docs/ai/KNOWN_PITFALLS.md` и reference-пакеты, но нет единой папки с минимальными «copy-with-context» примерами по слоям (controller/service/migration/tests). | **Берём**: это снижает галлюцинации по Loco/SeaORM и ускоряет codegen в рамках локальных паттернов проекта. | Создать `docs/ai/examples/` и добавить по одному короткому эталону на каждый тип операции; линковать из `docs/ai/SESSION_TEMPLATE.md`. |
| **Запрет `unwrap/expect/panic` через deny-lints вне тестов** | В CI уже есть `clippy -D warnings`, но нет глобального deny именно для `unwrap_used`/`expect_used`/`panic`. | **Берём** с rollout-подходом (сначала CI gate, затем жёсткий compile-time). | Добавить отдельный clippy-проход в CI с `-D clippy::unwrap_used -D clippy::expect_used -D clippy::panic` и подготовить список исключений для legacy-кода. |

**Почему такой выбор:** принцип «ошибка должна быть невозможной» принимается как базовый, но внедряется поэтапно, чтобы не сорвать текущий delivery (особенно для breaking-пунктов, затрагивающих все модули одновременно).

**Короткий ответ на вопрос «это всё, что нам надо оттуда?»** — **нет**: в ближайший цикл берём только минимально полезный набор, остальное оставляем в backlog.

**Берём сейчас (минимум):**
- `docs/ai/examples/` с живыми эталонами по слоям (низкий риск, быстрая отдача).
- CI-check для `clippy::unwrap_used`, `clippy::expect_used`, `clippy::panic` (через rollout с allowlist для legacy).
- Lightweight dependency-check для критичных модульных границ.

**Берём позже (после пилота/ADR):**
- `ServiceOutcome<T, E>` как обязательный контракт для mutating service-операций.
- `TenantScope`/scoped repositories как единый compile-time стандарт tenant-фильтрации.

**Пока не берём как отдельные инициативы:**
- Любые дубли уже существующих практик (`publish_in_tx`, текущая module/manifest validation) — только усиливаем точечно, без повторной документации теми же словами.
...
| 2.15 | Компиляционные guardrails для AI (ServiceOutcome/TenantScope/lints/examples) | 🟡 Важно | Backlog | Средняя | Correctness/Safety | Platform foundation + domain modules | Итерация 2-3 |
