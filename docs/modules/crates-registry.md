...
| `rustok-rbac` | RBAC/authorization-слой платформы, резолвинг разрешений и единый Casbin-backed runtime contract. | `RbacModule`, `PermissionResolver`, `RuntimePermissionResolver`, `PermissionAuthorizer`, `PermissionEvaluator`, `AuthzEngine`. | Нельзя внедрять собственные проверки ролей в `apps/server` в обход `rustok-rbac` (hardcoded role checks как substitute для authorizer). |
