...
- У каждого приложения/модуля/библиотеки **обязателен корневой `README.md`** с:
  - описанием назначения и принципа работы;
  - явным списком взаимодействий (с какими модулями/приложениями интегрируется);
  - ссылкой на локальную папку `docs/`.
